        ...
        case ESP_STATE_DATA_SEND:
            ...
            if (strstr(esp_rx_buf, expect_responses[cmd_index]) != NULL) {
                ...
                if (cmd_index == 7 && strstr(esp_rx_buf, "SEND OK") != NULL) {
                    drv_fan_open();  // 打开LED
                    led_status = true;
                    printf("FAN opened successfully!\r\n");
                }
...
void send_next_cmd(void) {
    if (cmd_index >= sizeof(cmds) / sizeof(cmds[0])) {
        ...
        // 关键改进：在发送关键指令前增加延时
        if (cmd_index == 1) drv_systick_ms(3000);  // 发送RESTORE前
    if (cmd_index == 5) drv_systick_ms(9000);  // 发送RESTORE前
    if (cmd_index == 6) drv_systick_ms(7000);  // 发送CWJAP_DEF前（WiFi连接）
