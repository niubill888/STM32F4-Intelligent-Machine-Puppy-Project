...
void USART3_IRQHandler(void)
{
	...
	{
		USART_SendData(USART6, USART_ReceiveData(USART3));
		
		USART_ClearITPendingBit(USART3, USART_IT_RXNE);
		
	}
	/*if(USART_GetITStatus(USART3, USART_IT_RXNE))//判断是不是接收串口的中断
	{
		USART_ClearITPendingBit(USART3, USART_IT_RXNE);
		// 接收一个字节数据
        if(i3 < sizeof(rcbuffer3) - 1)
        {
            rcbuffer3[i3] = (uint8_t)USART_ReceiveData(USART3);
            USART_SendData(USART6, rcbuffer3[i3]);
            i3++;
        }
        else
        {
	...
	/*else if(USART_GetITStatus(USART3, USART_IT_IDLE))
	{
		...
		if(i3 > 0)
        {
            rcbuffer3[i3] = '\0'; // 添加字符串结束符
            //rx_complete = 1;    // 设置接收完成标志
            //printf("%s\r\n",rcbuffer3);
			/*USART_SendData(USART6, rcbuffer3);
            for(int i = 0; i < i3; i++) {
                while(USART_GetFlagStatus(USART6, USART_FLAG_TXE) == RESET);
                USART_SendData(USART6, rcbuffer3[i]);
            }
            // 重置缓冲区索引
            i3 = 0;
		}
