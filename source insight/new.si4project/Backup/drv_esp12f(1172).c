...
void parse_tcp_command(const char* cmd) {
    ...
    switch (cmd_type) {
        ...
        // -------------------- 温度显示 --------------------
        case CMD_TEMP_SHOW:
            if      (strcmp(action, "open") == 0) software_trigger_key1() ;  // 需实现：开启温度显示
        ...
        // -------------------- 湿度显示 --------------------
        case CMD_HUMI_SHOW:
            if      (strcmp(action, "open") == 0)  ;  // 需实现：开启湿度显示
        ...
        // -------------------- 流水灯 --------------------
        case CMD_FLOW_LIGHT:
            if      (strcmp(action, "open") == 0)  drv_led_flow();    // 需实现：开启流水灯
            else if (strcmp(action, "close") == 0) drv_led_open();   // 需实现：关闭流水灯
...
void esp_state_rp(void)
{
		...
		// 错误状态处理：重置并重新开始
			...
			current_state = ESP_STATE_INIT;
			led_status = false; 	
			server_disconnected = false; // 重连时清除断开标志
			send_next_cmd();
		}
